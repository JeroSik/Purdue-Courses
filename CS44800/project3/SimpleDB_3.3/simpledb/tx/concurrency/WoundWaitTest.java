...
public class WoundWaitTest {
	...
	public static void main(String[] args) {
		...
		SimpleDB db = new SimpleDB("woundwaittest", 400, 8); 
		fm = db.fileMgr();
		lm = db.logMgr();
		bm = db.bufferMgr();
	...
	static class A implements Runnable { 
		public void run() {
			...
			try {
				...
				// Under wound-wait, txB gets aborted because txA will wound txB
				// for it to release its write lock on block 2. Uncomment
				// the following statement and txB will not get aborted.
//				            Thread.sleep(2000);
				txA.getInt(blk2, 0);
	...
	static class C implements Runnable {
		public void run() {
			...
			try {
				...
				System.out.println("Transaction C interrupted");
				txC.rollback();
			} catch(LockAbortException e) {
