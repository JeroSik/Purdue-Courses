...
query = (
    "select distinct m.title, round(avg(r.rating), 1) as rating "
    + "from movies as m join ratings as r on m.movieid = r.movieid "
    + "where m.movieid = r.movieid and m.title like '%(1998)%' "
    + "group by m.title"
)
...
results.rdd.map(lambda p: str(p.title) + "::" + str(p.rating)).repartition(10).saveAsTextFile(sys.argv[2])
