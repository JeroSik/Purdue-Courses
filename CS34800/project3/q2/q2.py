...
query = (
    ...
    + "where m.movieid = r.movieid and r.rating = 5 and m.title like '%2000%'"
)
results = sqlContext.sql(query)

# write data back to HDFS;
# repartition down to 10 partitions to reduce number of HDFS files written;
results.rdd.map(lambda p: str(p.title) + "::" + str(p.genres)).repartition(10).saveAsTextFile(sys.argv[2])
