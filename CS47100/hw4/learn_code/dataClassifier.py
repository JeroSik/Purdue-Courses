...
def enhancedFeatureExtractorDigit(datum):
    ...
    In addition to the basic feature, (identifying each pixel,) the number of separate connected
    regions of white pixels was considered. "1, 2, 3, 5, 7 tend to have one contiguous region of
    white space while the loops in 6, 8, 9 create more."

    Multiple contiguous regions of white space can be found by running a DFS on white pixel spots.
    Similar to "counting islands".
    ##
    """
    features =  basicFeatureExtractorDigit(datum)

    "*** YOUR CODE HERE ***"
    # Initialize visited set to store already seen pixels and number of connected regions
    visited = set()
    regionCount = 0
    ...
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            # Run BFS if pixel is white and has not been visited
            if not (x,y) in visited and datum.getPixel(x,y) == 0:
                BFS(datum, visited, (x,y))

                # Increase connected region count
                regionCount += 1

    # Check if there are multiple contigous regions
    if regionCount > 1:
        features["Multiple contigous regions"] = 1
    else:
        features["Multiple contigous regions"] = 0

    # Return enhanced features
    return features


def BFS(datum, visited, curr):
    """
    Helper funciton to run BFS given the datum, visited set, and current pixels.

    Uses the util.Queue() as the BFS queue.
    """
    # Initialize queue with passed in pixels and add pixel to visited set
    queue = util.Queue()
    queue.push(curr)
    visited.add(curr)

    # Iterate until no more neighboring white pixels
    while not queue.isEmpty():
        # Get current pixel's x and y values
        cord = queue.pop()
        x = cord[0]
        y = cord[1]

        # Check neighboring pixels in order of left, right, down, up
        # If valid pixel is white and not visited, then add to queue and visited set

        # LEFT
        if (x - 1 >= 0) and (datum.getPixel(x-1,y) == 0) and ((x-1,y) not in visited):
            visited.add((x-1,y))
            queue.push((x-1,y))
        # RIGHT
        if (x + 1 < DIGIT_DATUM_WIDTH) and (datum.getPixel(x+1,y) == 0) and ((x+1,y) not in visited):
            visited.add((x+1,y))
            queue.push((x+1,y))
        # DOWN
        if (y - 1 >= 0) and (datum.getPixel(x,y-1) == 0) and ((x,y-1) not in visited):
            visited.add((x,y-1))
            queue.push((x,y-1))
        # UP
        if (y + 1 < DIGIT_DATUM_HEIGHT) and (datum.getPixel(x,y+1) == 0) and ((x,y+1) not in visited):
            visited.add((x,y+1))
            queue.push((x,y+1))
