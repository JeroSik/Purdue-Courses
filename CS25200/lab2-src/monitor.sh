...
function generate_report {
  ...
  if [[ $REPORT_COUNT -eq $MAXIMUM_REPORTS ]]; then
    oldest_report=$(ls -1t ./reports_dir/ | tail -1)
    rm ./reports_dir/$oldest_report
...
function calculate_mem_usage
{
  ...
  mem_usage=$(cat /proc/$PID/status | /bin/egrep 'VmRSS' | awk '{print $2}')
...
function notify
{
  ...
  if [[ $MEM_THRESHOLD -gt 0 ]]; then
    if [[ $cpu_usage_int -gt $CPU_THRESHOLD || $mem_usage_int -gt $MEM_THRESHOLD ]]; then
      recent_report=$(ls -1t ./reports_dir/ | head -1)
      # cat ./reports_dir/$recent_report
      /usr/bin/mailx -s "REPORT: THRESHOLD EXCEEDED" $USER < ./reports_dir/$recent_report
    fi
  else
...
REPORT_COUNT=0 #Counts the number of reports in /reports_dir
